...
module commit_stage (
    ...
    output logic               commit_store_req_o  
);
    ...
    always_comb begin
        ...
        commit_en_o        = 1'b0;
        commit_store_req_o = 1'b0;

        if (commit_valid_i && rob_head_entry_i.busy && rob_head_entry_i.is_complete) begin
            ...
            if (rob_head_entry_i.inst_data.instr_type == INSTR_STORE) begin
                commit_store_req_o = 1'b1; 
                if (lsq_ready_to_commit_i) begin
                    commit_en_o = 1'b1;   
                end
            end else begin
                commit_en_o = 1'b1;
