module memory_stage (
    ...
    typedef enum logic [1:0] { IDLE, ACCESS, DONE } state_t;
    state_t current_state, next_state;

    logic [9:0]  address_reg; 
    logic [31:0] write_data_reg; 
    logic        is_read_op_reg;
    ...
    always_ff @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            if (current_state == IDLE && (mem_read_en || mem_write_en)) begin
                address_reg     <= mem_address_in; 
                write_data_reg  <= mem_write_data_in; 
                is_read_op_reg  <= mem_read_en;
            end
            
            if (current_state == ACCESS && is_read_op_reg && ram_done) begin
    ...
    always_comb begin
        ...
        read_write_req         = 1'b0;
        data_mem_read_write    = is_read_op_reg;
        ram_address_out        = address_reg; 
        ram_write_data_out     = write_data_reg; 
