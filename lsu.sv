...
module load_store_unit #(
    ...
    assign do_retire  = lsq[head].valid && lsq[head].executed && 
                        (lsq[head].is_load ? (!mem_busy || (lsq[head].rob_tag != mem_active_rob_tag)) 
                                           : lsq_store_done_o);
    ...
    always_ff @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            if (commit_store_req_i && !mem_busy && !store_commit_active) begin
                if (lsq[head].valid && !lsq[head].is_load && lsq[head].executed) begin
                    mem_busy            <= 1'b1;
                    store_commit_active <= 1'b1;
                    data_mem_req        <= 1'b1;
                    data_mem_read_write <= 1'b0; 
                    data_mem_addr       <= lsq[head].addr;
                    data_mem_write_data <= lsq[head].data;
                end
            end
            ...
            if (!mem_busy && issue_found) begin
                ...
                if (!lsq[issue_idx].is_load) begin
                    lsu_cdb_valid   <= 1'b1;
                    lsu_cdb_rob_tag <= lsq[issue_idx].rob_tag;
                    lsu_cdb_value   <= 32'd0;
                end else if (do_forwarding) begin
                    lsu_cdb_valid   <= 1'b1;
                    lsu_cdb_rob_tag <= lsq[issue_idx].rob_tag;
